...
public class SegmentTree {
    ...
    private long rMaxQ(int i, int i2, int i3) {
        Node node = this.heap[i];
        if (node.pendingVal != null && contains(node.from, node.f1461to, i2, i3)) {
            return node.pendingVal.intValue();
        }
        if (contains(i2, i3, node.from, node.f1461to)) {
            return this.heap[i].max;
        }
        if (!intersects(i2, i3, node.from, node.f1461to)) {
            return 0L;
        }
        propagate(i);
        int i4 = i * 2;
    ...
    static class Node {
        ...
        int size() {
            return (this.f1461to - this.from) + 1;
        }
    }
}
