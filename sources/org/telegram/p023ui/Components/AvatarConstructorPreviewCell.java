...
public abstract class AvatarConstructorPreviewCell extends FrameLayout {
    ...
    public AvatarConstructorPreviewCell(Context context, boolean z) {
        ...
        if (tL_emojiList == null || tL_emojiList.document_id.isEmpty()) {
            ...
            if (stickerSets.isEmpty()) {
                ...
                for (int i2 = 0; i2 < featuredEmojiSets.size(); i2++) {
                    ...
                    if (document != null) {
                        this.emojiList.document_id.add(Long.valueOf(document.f1579id));
                    } else if (stickerSetCovered instanceof TLRPC.TL_stickerSetFullCovered) {
                        TLRPC.TL_stickerSetFullCovered tL_stickerSetFullCovered = (TLRPC.TL_stickerSetFullCovered) stickerSetCovered;
                        if (!tL_stickerSetFullCovered.documents.isEmpty()) {
                            this.emojiList.document_id.add(Long.valueOf(((TLRPC.Document) tL_stickerSetFullCovered.documents.get(0)).f1579id));
                        }
                    }
                }
            } else {
                for (int i3 = 0; i3 < stickerSets.size(); i3++) {
                    ...
                    if (!tL_messages_stickerSet.documents.isEmpty()) {
                        this.emojiList.document_id.add(Long.valueOf(((TLRPC.Document) tL_messages_stickerSet.documents.get(Math.abs(Utilities.fastRandom.nextInt() % tL_messages_stickerSet.documents.size()))).f1579id));
                    }
                }
            }
        }
        ...
        this.textView.setText(LocaleController.getString(C2369R.string.UseEmoji));
