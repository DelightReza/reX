...
public class SpeedLineParticles$Drawable {
    ...
    /* renamed from: dt */
    private final float f1947dt = 1000.0f / AndroidUtilities.screenRefreshRate;
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < this.particles.size(); i++) {
            ...
            if (jCurrentTimeMillis > particle.lifeTime || !this.screenRect.contains(particle.f1948x, particle.f1949y)) {
    ...
    private class Particle {
        ...
        public void draw(Canvas canvas, int i, long j) {
            int i2 = i * 4;
            SpeedLineParticles$Drawable.this.lines[i2] = this.f1948x;
            SpeedLineParticles$Drawable.this.lines[i2 + 1] = this.f1949y;
            SpeedLineParticles$Drawable.this.lines[i2 + 2] = this.f1948x + (AndroidUtilities.m1146dp(30.0f) * this.vecX);
            SpeedLineParticles$Drawable.this.lines[i2 + 3] = this.f1949y + (AndroidUtilities.m1146dp(30.0f) * this.vecY);
            if (SpeedLineParticles$Drawable.this.paused) {
                return;
            }
            float fM1146dp = AndroidUtilities.m1146dp(4.0f) * (SpeedLineParticles$Drawable.this.f1947dt / 660.0f);
            SpeedLineParticles$Drawable speedLineParticles$Drawable = SpeedLineParticles$Drawable.this;
            float f = fM1146dp * speedLineParticles$Drawable.speedScale;
            this.f1948x += this.vecX * f;
            this.f1949y += this.vecY * f;
            float f2 = this.inProgress;
            if (f2 != 1.0f) {
                float f3 = f2 + (speedLineParticles$Drawable.f1947dt / 200.0f);
                this.inProgress = f3;
                if (f3 > 1.0f) {
        ...
        public void genPosition(long j, boolean z) {
            ...
            this.f1948x = fAbs;
            this.f1949y = fAbs2;
            double dAtan2 = Math.atan2(fAbs - SpeedLineParticles$Drawable.this.rect.centerX(), this.f1949y - SpeedLineParticles$Drawable.this.rect.centerY());
