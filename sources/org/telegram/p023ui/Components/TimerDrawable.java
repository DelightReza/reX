...
public class TimerDrawable extends Drawable {
    ...
    public void setTime(int i) {
        ...
        if (this.time != i) {
            this.time = i;
            if (this.isDialog) {
                this.currentTtlIcon = ContextCompat.getDrawable(this.context, C2369R.drawable.msg_autodelete_badge2).mutate();
            } else {
                Drawable drawableMutate = ContextCompat.getDrawable(this.context, i == 0 ? C2369R.drawable.msg_mini_autodelete : C2369R.drawable.msg_mini_autodelete_empty).mutate();
            ...
            if (i2 >= 1 && i2 < 60) {
                str = "" + i;
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2369R.string.SecretChatTimerSeconds);
                }
            } else if (i2 >= 60 && i2 < 3600) {
                str = "" + (i / 60);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2369R.string.SecretChatTimerMinutes);
                }
            } else if (i2 >= 3600 && i2 < 86400) {
                str = "" + ((i / 60) / 60);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2369R.string.SecretChatTimerHours);
                }
            } else if (i2 >= 86400 && i2 < 604800) {
                str = "" + (((i / 60) / 60) / 24);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2369R.string.SecretChatTimerDays);
                }
            } else if (i2 < 2678400) {
                str = "" + ((((i / 60) / 60) / 24) / 7);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2369R.string.SecretChatTimerWeeks);
                ...
                str = "" + ((((i / 60) / 60) / 24) / 30);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2369R.string.SecretChatTimerMonths);
                }
            } else {
                str = "" + ((((i / 60) / 60) / 24) / 364);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2369R.string.SecretChatTimerYears);
                }
            }
            String str2 = str;
            this.timePaint.setTextSize(AndroidUtilities.m1146dp(11.0f));
            float fMeasureText = this.timePaint.measureText(str2);
            this.timeWidth = fMeasureText;
            if (fMeasureText > AndroidUtilities.m1146dp(13.0f)) {
                this.timePaint.setTextSize(AndroidUtilities.m1146dp(9.0f));
                this.timeWidth = this.timePaint.measureText(str2);
            }
            if (this.timeWidth > AndroidUtilities.m1146dp(13.0f)) {
                this.timePaint.setTextSize(AndroidUtilities.m1146dp(6.0f));
                this.timeWidth = this.timePaint.measureText(str2);
            }
            try {
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.currentTtlIcon != null) {
            ...
            if (this.isDialog) {
                ...
                rect.set(getBounds().centerX() - AndroidUtilities.m1146dp(10.5f), getBounds().centerY() - AndroidUtilities.m1146dp(10.5f), (getBounds().centerX() - AndroidUtilities.m1146dp(10.5f)) + this.currentTtlIcon.getIntrinsicWidth(), (getBounds().centerY() - AndroidUtilities.m1146dp(10.5f)) + this.currentTtlIcon.getIntrinsicHeight());
    ...
    public int getIntrinsicWidth() {
        return AndroidUtilities.m1146dp(23.0f);
    }
