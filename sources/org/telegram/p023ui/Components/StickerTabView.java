...
public class StickerTabView extends FrameLayout {
    ...
    public StickerTabView(Context context, int i) {
        ...
        if (i == 2) {
            ...
            this.imageView.setRoundRadius(AndroidUtilities.m1146dp(6.0f));
    ...
    public void setExpanded(boolean z) {
        ...
        this.visibleView.getLayoutParams().width = AndroidUtilities.m1146dp(z ? f2 : f);
        ViewGroup.LayoutParams layoutParams = this.visibleView.getLayoutParams();
        if (z) {
            f = f2;
        }
        layoutParams.height = AndroidUtilities.m1146dp(f);
        this.textView.setVisibility(z ? 0 : 8);
        if (this.type == 1 || !this.roundImage) {
            return;
        }
        this.imageView.setRoundRadius(AndroidUtilities.m1146dp(this.visibleView.getLayoutParams().width / 2.0f));
    }
    ...
    public void updateExpandProgress(float f) {
        ...
        if (this.expanded) {
            ...
            float f4 = 1.0f - f;
            this.visibleView.setTranslationY((((AndroidUtilities.m1146dp(36.0f - f2) / 2.0f) - (AndroidUtilities.m1146dp(86.0f - f3) / 2.0f)) * f4) - (AndroidUtilities.m1146dp(8.0f) * f));
            this.visibleView.setTranslationX(((AndroidUtilities.m1146dp(33.0f - f2) / 2.0f) - (AndroidUtilities.m1146dp(ScrollSlidingTabStrip.EXPANDED_WIDTH - f3) / 2.0f)) * f4);
            this.textView.setAlpha(Math.max(0.0f, (f - 0.5f) / 0.5f));
            this.textView.setTranslationY((-AndroidUtilities.m1146dp(40.0f)) * f4);
            this.textView.setTranslationX((-AndroidUtilities.m1146dp(12.0f)) * f4);
