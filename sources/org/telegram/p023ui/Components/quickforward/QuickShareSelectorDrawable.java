...
public class QuickShareSelectorDrawable extends Drawable implements Animator.AnimatorListener {
    ...
    public int getBubbleWidth() {
        int i = Sizes.AVATAR;
        int i2 = Sizes.GAP;
        return AndroidUtilities.m1146dp((((i + i2) * this.avatarCells.length) - i2) + (Sizes.PADDING_H * 2));
    }
    ...
    public QuickShareSelectorDrawable(QuickShareSelectorOverlayLayout quickShareSelectorOverlayLayout, ChatMessageCell chatMessageCell, List list, String str, Runnable runnable) {
        ...
        while (true) {
            ...
            if (i < quickShareAvatarCellArr.length) {
                ...
                this.shadowDrawable = ContextCompat.getDrawable(quickShareSelectorOverlayLayout.getContext(), C2369R.drawable.reactions_bubble_shadow).mutate();
    ...
    private void calculateCords() {
        ...
        float sideButtonStartX = this.offsetX + this.cell.getSideButtonStartX() + fM1146dp;
        float sideButtonStartY = this.offsetY + this.cell.getSideButtonStartY() + fM1146dp;
        this.bubbleStart.set(sideButtonStartX - fM1146dp, sideButtonStartY - fM1146dp, sideButtonStartX + fM1146dp, sideButtonStartY + fM1146dp);
        float fM1146dp2 = AndroidUtilities.m1146dp(16.0f);
        if (this.bubbleStart.right + AndroidUtilities.m1146dp(48.0f) + fM1146dp2 > this.parent.getMeasuredWidth()) {
            this.bubbleOffset = Math.max(0.0f, (this.parent.getMeasuredWidth() - fM1146dp2) - this.bubbleStart.right);
        } else if (((this.bubbleStart.right + AndroidUtilities.m1146dp(48.0f)) - getBubbleWidth()) - fM1146dp2 < 0.0f) {
            this.bubbleOffset = Math.max(0.0f, (fM1146dp2 + getBubbleWidth()) - this.bubbleStart.right);
        } else {
            this.bubbleOffset = AndroidUtilities.m1146dp(48.0f);
        }
    }
    ...
    public void onTouchMoveEvent(float f, float f2) {
        if (this.openAnimationCompleted) {
            ...
            setIndex((((float) (-AndroidUtilities.m1146dp((float) (Sizes.TEXT_PADDING_EXTERNAL + 21)))) >= f4 || f4 >= this.bubbleCurrent.height()) ? -1 : MathUtils.clamp((int) Math.floor((f3 - (AndroidUtilities.m1146dp(Sizes.PADDING_H) - (AndroidUtilities.m1146dp(Sizes.GAP) / 2.0f))) / AndroidUtilities.m1146dp(Sizes.AVATAR + Sizes.GAP)), 0, this.avatarCells.length - 1));
        }
    }
    ...
    public void closeImpl() {
        ...
        if (this.openAnimationCompleted && !this.isDestroyed) {
            ...
            blurVisibilityDrawable.render((int) this.bubbleCurrent.width(), (int) (this.bubbleCurrent.height() + AndroidUtilities.m1146dp(30.0f)), AndroidUtilities.m1146dp(Sizes.BLUR_RADIUS), 4.0f);
        }
        invalidateSelf();
    }
    ...
    public void drawBubble(Canvas canvas, int i) {
        canvas.save();
        RectF rectF = this.bubbleCurrent;
        canvas.translate(-rectF.left, (-rectF.top) + AndroidUtilities.m1146dp(30.0f));
        draw(canvas, i, true);
        canvas.restore();
    }
    ...
    public void draw(Canvas canvas, int i, boolean z) {
        ...
        if (blurVisibilityDrawable != null && !z) {
            ...
            int iM1146dp = (int) (rectF.top - AndroidUtilities.m1146dp(30.0f));
            RectF rectF2 = this.bubbleCurrent;
            blurVisibilityDrawable.setBounds(i5, iM1146dp, (int) rectF2.right, (int) rectF2.bottom);
            ...
            if (this.selectedIndex != -1) {
                ...
                float fCenterX = this.bubbleCurrent.centerX() + (AndroidUtilities.m1146dp(Sizes.AVATAR + Sizes.GAP) * (this.selectedIndex - 2));
                ...
                if (lottieLayout != null && lottieLayout.top) {
                    fMin = Math.max(fCenterY, f5) + AndroidUtilities.m1146dp(Sizes.CLOSE_AVATAR_JUMP_HEIGHT);
                } else {
                    fMin = Math.min(fCenterY, f5) - AndroidUtilities.m1146dp(Sizes.CLOSE_AVATAR_JUMP_HEIGHT);
                }
                this.avatarCells[this.selectedIndex].drawBlurredAvatar(canvas2, fromTo(fCenterX, f4, interpolation2), findParabola(fCenterX, fCenterY, f4, f5, f6, fMin, interpolation2), fromTo((AndroidUtilities.m1146dp(Sizes.AVATAR) / 2.0f) + AndroidUtilities.m1146dp(2.0f), AndroidUtilities.m1146dp(12.0f), interpolation2), interpolation);
                return;
            }
            return;
        }
        ...
        rectF3.inset(-AndroidUtilities.m1146dp(8.0f), -AndroidUtilities.m1146dp(8.0f));
        Rect rect = tmpRect;
        rectF3.round(rect);
        ...
        if (!this.openAnimationCompleted) {
            ...
            canvas2.rotate(interpolation3, AndroidUtilities.m1146dp(16.0f), AndroidUtilities.m1146dp(16.0f));
        ...
        float fM1146dp = AndroidUtilities.m1146dp(2.0f) * Interpolators.avatarOvershootCancel.getInterpolation(this.openProgress);
        float fM1146dp2 = ((AndroidUtilities.m1146dp(Sizes.AVATAR + 2) * Interpolators.avatar1.getInterpolation(this.openProgress)) / 2.0f) - fM1146dp;
        float fM1146dp3 = ((AndroidUtilities.m1146dp(Sizes.AVATAR + 2) * Interpolators.avatar2.getInterpolation(this.openProgress)) / 2.0f) - fM1146dp;
        float fM1146dp4 = ((AndroidUtilities.m1146dp(Sizes.AVATAR + 2) * Interpolators.avatar3.getInterpolation(this.openProgress)) / 2.0f) - fM1146dp;
        int i6 = 0;
        while (i6 < i4) {
            int i7 = 0;
            while (true) {
                if (i7 < this.avatarCells.length) {
                    if (!(i6 == 0 && i7 == this.selectedIndex) && (i6 != 1 || i7 == this.selectedIndex)) {
                        ...
                        float f8 = i7 == i4 ? fM1146dp2 : (i7 == 1 || i7 == 3) ? fM1146dp3 : fM1146dp4;
                        float fCenterX2 = this.bubbleCurrent.centerX() + (AndroidUtilities.m1146dp(Sizes.AVATAR + Sizes.GAP) * length);
                        float fCenterY2 = this.bubbleCurrent.centerY();
                        QuickShareAvatarCell quickShareAvatarCell2 = this.avatarCells[i7];
                        float fM1146dp5 = AndroidUtilities.m1146dp(Sizes.TEXT_PADDING_EXTERNAL);
                        float measuredWidth = this.parent.getMeasuredWidth() - AndroidUtilities.m1146dp(Sizes.TEXT_PADDING_EXTERNAL);
    ...
    private void updateColors() {
        ...
        LinearGradient linearGradient = new LinearGradient(0.0f, 0.0f, 0.0f, AndroidUtilities.m1146dp(100.0f), new int[]{color, 16777215 & color}, new float[]{0.0f, 1.0f}, Shader.TileMode.CLAMP);
    ...
    public void calculateOpeningAnimationPositions() {
        ...
        float interpolation2 = (Interpolators.buttonJumpUp.getInterpolation(this.openProgress) - Interpolators.buttonJumpDown.getInterpolation(this.openProgress)) * AndroidUtilities.m1146dp(13.0f);
        this.buttonCurrent.set(this.bubbleStart);
        this.buttonCurrent.offset(0.0f, -interpolation2);
        float fFromTo = fromTo(this.buttonCurrent.height(), AndroidUtilities.m1146dp(Sizes.BUBBLE_HEIGHT) + (AndroidUtilities.m1146dp(2.0f) * interpolation), Interpolators.heightExpansion.getInterpolation(this.openProgress)) / 2.0f;
        Interpolator interpolator = Interpolators.widthExpansion;
        float fFromTo2 = fromTo(this.buttonCurrent.width(), getBubbleWidth() + (AndroidUtilities.m1146dp(10.0f) * interpolation), interpolator.getInterpolation(this.openProgress));
        float f = fFromTo * 2.0f;
        float fMax = Math.max(fFromTo2, f);
        float fCenterX = this.bubbleStart.centerX() + fFromTo + (Math.min(AndroidUtilities.m1146dp(-12.0f) + this.bubbleOffset, (fMax - Math.max(this.buttonCurrent.width(), f)) / 2.0f) * interpolator.getInterpolation(this.openProgress));
        float fM1146dp = ((this.bubbleStart.bottom - fFromTo) - 1.0f) - ((AndroidUtilities.m1146dp(38.0f) + (AndroidUtilities.m1146dp(6.0f) * interpolation)) * Interpolators.bubbleY.getInterpolation(this.openProgress));
        RectF rectF = this.bubbleCurrent;
        rectF.left = fCenterX - fMax;
        rectF.top = fM1146dp - fFromTo;
        rectF.right = fCenterX;
        rectF.bottom = fM1146dp + fFromTo;
        if (!this.ballsAllowed || this.openAnimationCompleted) {
            return;
        }
        float fFromTo3 = fromTo(AndroidUtilities.m1146dp(5.0f), AndroidUtilities.m1146dp(3.0f), Interpolators.ballsRadius.getInterpolation(this.openProgress));
    ...
    public static Interpolator interpolator(final Interpolator interpolator, int i, int i2, int i3, final boolean z) {
        ...
        return new Interpolator() { // from class: org.telegram.ui.Components.quickforward.QuickShareSelectorDrawable$$ExternalSyntheticLambda2
            ...
            public final float getInterpolation(float f4) {
                return QuickShareSelectorDrawable.m12276$r8$lambda$TFdbEg5B8rng8gyNxj41ZbRLAs(z, f2, f3, interpolator, f4);
            }
        };
    }

    /* renamed from: $r8$lambda$TFdbEg5B8-rng8gyNxj41ZbRLAs, reason: not valid java name */
    public static /* synthetic */ float m12276$r8$lambda$TFdbEg5B8rng8gyNxj41ZbRLAs(boolean z, float f, float f2, Interpolator interpolator, float f3) {
        if (z) {
