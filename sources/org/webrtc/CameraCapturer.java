...
abstract class CameraCapturer implements CameraVideoCapturer {
    ...
    private final CameraSession.CreateSessionCallback createSessionCallback = new CameraSession.CreateSessionCallback() { // from class: org.webrtc.CameraCapturer.1
        ...
        public void onDone(CameraSession cameraSession) {
            ...
            Logging.m1368d(CameraCapturer.TAG, "Create session done. Switch state: " + CameraCapturer.this.switchState);
            ...
            synchronized (CameraCapturer.this.stateLock) {
                try {
                    ...
                    if (CameraCapturer.this.openAttemptsRemaining <= 0) {
                        Logging.m1372w(CameraCapturer.TAG, "Opening camera failed, passing: " + str);
                        CameraCapturer.this.sessionOpening = false;
                        CameraCapturer.this.stateLock.notifyAll();
                        ...
                        Logging.m1372w(CameraCapturer.TAG, "Opening camera failed, retry: " + str);
    ...
    private final CameraSession.Events cameraSessionEventsHandler = new CameraSession.Events() { // from class: org.webrtc.CameraCapturer.2
            ...
            synchronized (CameraCapturer.this.stateLock) {
                try {
                    if (CameraCapturer.this.currentSession != null) {
                        Logging.m1372w(CameraCapturer.TAG, "onCameraOpening while session was open.");
                    } else {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                try {
                    if (cameraSession != CameraCapturer.this.currentSession) {
                        Logging.m1372w(CameraCapturer.TAG, "onCameraError from another session: " + str);
                        return;
                    }
                    CameraCapturer.this.eventsHandler.onCameraError(str);
        ...
        public void onCameraDisconnected(CameraSession cameraSession) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                try {
                    if (cameraSession != CameraCapturer.this.currentSession) {
                        Logging.m1372w(CameraCapturer.TAG, "onCameraDisconnected from another session.");
                    } else {
                        CameraCapturer.this.eventsHandler.onCameraDisconnected();
                        CameraCapturer.this.stopCapture();
                    }
            ...
            synchronized (CameraCapturer.this.stateLock) {
                try {
                    if (cameraSession != CameraCapturer.this.currentSession && CameraCapturer.this.currentSession != null) {
                        Logging.m1368d(CameraCapturer.TAG, "onCameraClosed from another session.");
                    } else {
        ...
        public void onFrameCaptured(CameraSession cameraSession, VideoFrame videoFrame) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                try {
                    if (cameraSession != CameraCapturer.this.currentSession) {
                        Logging.m1372w(CameraCapturer.TAG, "onFrameCaptured from another session.");
                        return;
                    }
    ...
    public void startCapture(int i, int i2, int i3) {
        Logging.m1368d(TAG, "startCapture: " + i + "x" + i2 + "@" + i3);
        ...
        synchronized (this.stateLock) {
            try {
                ...
                Logging.m1372w(TAG, "Session already open");
            } catch (Throwable th) {
                throw th;
            }
        }
    ...
    public void stopCapture() {
        Logging.m1368d(TAG, "Stop capture");
        synchronized (this.stateLock) {
            while (this.sessionOpening) {
                Logging.m1368d(TAG, "Stop capture: Waiting for session to open");
                try {
                    this.stateLock.wait();
                } catch (InterruptedException unused) {
                    Logging.m1372w(TAG, "Stop capture interrupted while waiting for the session to open.");
                    Thread.currentThread().interrupt();
                    return;
                }
            }
            if (this.currentSession != null) {
                Logging.m1368d(TAG, "Stop capture: Nulling session");
                ...
                Logging.m1368d(TAG, "Stop capture: No session open");
            }
        }
        Logging.m1368d(TAG, "Stop capture done");
    }

    @Override // org.webrtc.VideoCapturer
    public void changeCaptureFormat(int i, int i2, int i3) {
        Logging.m1368d(TAG, "changeCaptureFormat: " + i + "x" + i2 + "@" + i3);
        synchronized (this.stateLock) {
            stopCapture();
    ...
    @Override // org.webrtc.VideoCapturer
    public void dispose() {
        Logging.m1368d(TAG, "dispose");
        stopCapture();
    }

    @Override // org.webrtc.CameraVideoCapturer
    public void switchCamera(final CameraVideoCapturer.CameraSwitchHandler cameraSwitchHandler) {
        Logging.m1368d(TAG, "switchCamera");
    ...
    public void printStackTrace() {
        ...
        if (thread != null) {
            ...
            if (stackTrace.length > 0) {
                Logging.m1368d(TAG, "CameraCapturer stack trace:");
                for (StackTraceElement stackTraceElement : stackTrace) {
                    Logging.m1368d(TAG, stackTraceElement.toString());
                }
            }
        }
    }
    ...
    public void switchCameraInternal(CameraVideoCapturer.CameraSwitchHandler cameraSwitchHandler, String str) {
        Logging.m1368d(TAG, "switchCamera internal");
        ...
        synchronized (this.stateLock) {
            try {
                ...
                Logging.m1368d(TAG, "switchCamera: Stopping session");
                this.cameraStatistics.release();
                this.cameraStatistics = null;
                final CameraSession cameraSession = this.currentSession;
                ...
                createSessionInternal(0);
                Logging.m1368d(TAG, "switchCamera done");
            } catch (Throwable th) {
                throw th;
            }
        }
    ...
    public void checkIsOnCameraThread() {
        ...
        Logging.m1369e(TAG, "Check is on camera thread failed.");
