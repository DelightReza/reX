...
public class WebRtcAudioManager {
    ...
    public static synchronized void setStereoOutput(boolean z) {
        Logging.m1372w(TAG, "Overriding default output behavior: setStereoOutput(" + z + ')');
        useStereoOutput = z;
    }

    public static synchronized void setStereoInput(boolean z) {
        Logging.m1372w(TAG, "Overriding default input behavior: setStereoInput(" + z + ')');
        useStereoInput = z;
    }
    ...
    private static class VolumeLogger {
        ...
        private class LogVolumeTask extends TimerTask {
            ...
            public void run() {
                ...
                if (mode == 1) {
                    Logging.m1368d(WebRtcAudioManager.TAG, "STREAM_RING stream volume: " + VolumeLogger.this.audioManager.getStreamVolume(2) + " (max=" + this.maxRingVolume + ")");
                    return;
                }
                if (mode == 3) {
                    Logging.m1368d(WebRtcAudioManager.TAG, "VOICE_CALL stream volume: " + VolumeLogger.this.audioManager.getStreamVolume(0) + " (max=" + this.maxVoiceCallVolume + ")");
                }
            }
        }
    ...
    private boolean init() {
        Logging.m1368d(TAG, "init" + WebRtcAudioUtils.getThreadInfo());
        if (this.initialized) {
            return true;
        }
        Logging.m1368d(TAG, "audio mode is: " + WebRtcAudioUtils.modeToString(this.audioManager.getMode()));
    ...
    private void dispose() {
        Logging.m1368d(TAG, "dispose" + WebRtcAudioUtils.getThreadInfo());
        if (this.initialized) {
    ...
    private boolean isDeviceBlacklistedForOpenSLESUsage() {
        ...
        Logging.m1368d(TAG, Build.MODEL + " is blacklisted for OpenSL ES usage!");
        return true;
    }
    ...
    private boolean isAAudioSupported() {
        Logging.m1372w(TAG, "AAudio support is currently disabled on all devices!");
        return false;
    }

    private int getNativeOutputSampleRate() {
        if (WebRtcAudioUtils.runningOnEmulator()) {
            Logging.m1368d(TAG, "Running emulator, overriding sample rate to 8 kHz.");
            return 8000;
        }
        if (WebRtcAudioUtils.isDefaultSampleRateOverridden()) {
            Logging.m1368d(TAG, "Default sample rate is overriden to " + WebRtcAudioUtils.getDefaultSampleRateHz() + " Hz");
            return WebRtcAudioUtils.getDefaultSampleRateHz();
        }
        int sampleRateForApiLevel = getSampleRateForApiLevel();
        Logging.m1368d(TAG, "Sample rate is set to " + sampleRateForApiLevel + " Hz");
