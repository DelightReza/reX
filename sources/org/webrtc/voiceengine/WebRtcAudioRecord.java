...
public class WebRtcAudioRecord {
    ...
    public static void setErrorCallback(WebRtcAudioRecordErrorCallback webRtcAudioRecordErrorCallback) {
        Logging.m1368d(TAG, "Set error callback");
    ...
    private class AudioRecordThread extends Thread {
        ...
        public void run() throws IllegalStateException, SecurityException, IllegalArgumentException {
            ...
            try {
                ...
                Logging.m1369e(WebRtcAudioRecord.TAG, "AudioRecord.stop failed: " + e2.getMessage());
            }
        }
    ...
    private boolean enableBuiltInAEC(boolean z) {
        ...
        if (webRtcAudioEffects == null) {
            Logging.m1369e(TAG, "Built-in AEC is not supported on this platform");
            return false;
        }
    ...
    private boolean enableBuiltInNS(boolean z) {
        ...
        WebRtcAudioEffects webRtcAudioEffects = this.effects;
        if (webRtcAudioEffects == null) {
            Logging.m1369e(TAG, "Built-in NS is not supported on this platform");
            return false;
        }
        ...
        if (this.captureType == 1) {
            if (Build.VERSION.SDK_INT >= 29) {
                try {
                    ...
                    WebRtcAudioRecord$$ExternalSyntheticApiModelOutline1.m1380m();
                    AudioPlaybackCaptureConfiguration.Builder builderM1379m = WebRtcAudioRecord$$ExternalSyntheticApiModelOutline0.m1379m(mediaProjection);
                    builderM1379m.addMatchingUsage(1);
                    builderM1379m.addMatchingUsage(14);
                    builderM1379m.addMatchingUsage(0);
                    WebRtcAudioRecord$$ExternalSyntheticApiModelOutline2.m1381m();
                    AudioRecord.Builder builderM1377m = org.webrtc.audio.WebRtcAudioRecord$$ExternalSyntheticApiModelOutline0.m1377m();
                    builderM1377m.setAudioPlaybackCaptureConfig(builderM1379m.build());
                    builderM1377m.setAudioFormat(new AudioFormat.Builder().setChannelMask(iChannelCountToConfiguration).setSampleRate(i).setEncoding(2).build());
                    builderM1377m.setBufferSizeInBytes(iMax);
                    this.audioRecord = builderM1377m.build();
                } catch (Throwable th) {
    ...
    public void initDeviceAudioRecord(MediaProjection mediaProjection) {
        ...
        try {
            ...
            AudioRecord.Builder builderM1377m = org.webrtc.audio.WebRtcAudioRecord$$ExternalSyntheticApiModelOutline0.m1377m();
            builderM1377m.setAudioPlaybackCaptureConfig(builderM1379m.build());
            builderM1377m.setAudioFormat(new AudioFormat.Builder().setChannelMask(iChannelCountToConfiguration).setSampleRate(this.requestedSampleRate).setEncoding(2).build());
            builderM1377m.setBufferSizeInBytes(iMax);
            AudioRecord audioRecordBuild = builderM1377m.build();
    ...
    public static synchronized void setAudioSource(int i) {
        Logging.m1372w(TAG, "Audio source is changed from: " + audioSource + " to " + i);
        audioSource = i;
    }
    ...
    private void releaseAudioResources(boolean z) {
        Logging.m1368d(TAG, "releaseAudioResources " + z);
        if (z) {
            AudioRecord audioRecord = this.deviceAudioRecord;
            if (audioRecord != null) {
                audioRecord.release();
