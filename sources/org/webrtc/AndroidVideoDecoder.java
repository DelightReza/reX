...
class AndroidVideoDecoder implements VideoDecoder, VideoSink {
    ...
    AndroidVideoDecoder(MediaCodecWrapperFactory mediaCodecWrapperFactory, String str, VideoCodecMimeType videoCodecMimeType, int i, EglBase.Context context) {
        ...
        Logging.m1368d(TAG, "ctor name: " + str + " type: " + videoCodecMimeType + " color format: " + i + " context: " + context);
    ...
    private VideoCodecStatus initDecodeInternal(int i, int i2) {
        ...
        Logging.m1368d(TAG, "initDecodeInternal name: " + this.codecName + " type: " + this.codecType + " width: " + i + " height: " + i2 + " color format: " + this.colorFormat);
        if (this.outputThread != null) {
            Logging.m1369e(TAG, "initDecodeInternal called while the codec is already running");
        ...
        try {
            ...
            try {
                ...
                Logging.m1368d(TAG, "initDecodeInternal done");
                return VideoCodecStatus.OK;
            } catch (IllegalArgumentException e) {
                e = e;
                Logging.m1370e(TAG, "initDecode failed", e);
                release();
                return VideoCodecStatus.FALLBACK_SOFTWARE;
            ...
            Logging.m1369e(TAG, "Cannot create media decoder " + this.codecName);
            return VideoCodecStatus.FALLBACK_SOFTWARE;
        }
    }
    ...
    public VideoCodecStatus decode(EncodedImage encodedImage, VideoDecoder.DecodeInfo decodeInfo) {
        ...
        if (byteBuffer == null) {
            Logging.m1369e(TAG, "decode() - no input data");
            return VideoCodecStatus.ERR_PARAMETER;
        }
        int iRemaining = byteBuffer.remaining();
        if (iRemaining == 0) {
            Logging.m1369e(TAG, "decode() - input buffer empty");
            return VideoCodecStatus.ERR_PARAMETER;
        }
    ...
    private VideoCodecStatus releaseInternal() {
        if (!this.running) {
            Logging.m1368d(TAG, "release: Decoder is not running.");
            return VideoCodecStatus.OK;
        }
        try {
            this.running = false;
            if (!ThreadUtils.joinUninterruptibly(this.outputThread, 5000L)) {
                Logging.m1370e(TAG, "Media decoder release timeout", new RuntimeException());
                return VideoCodecStatus.TIMEOUT;
            }
            if (this.shutdownException != null) {
                Logging.m1370e(TAG, "Media decoder release error", new RuntimeException(this.shutdownException));
    ...
    protected void deliverDecodedFrame() throws Throwable {
        ...
        try {
            ...
            if (iDequeueOutputBuffer < 0) {
                Logging.m1371v(TAG, "dequeueOutputBuffer returned " + iDequeueOutputBuffer);
                return;
            }
            ...
            Logging.m1370e(TAG, "deliverDecodedFrame failed", e);
        }
    }
    ...
    private void deliverByteFrame(int i, MediaCodec.BufferInfo bufferInfo, int i2, Integer num) throws Throwable {
        ...
        if (i7 < ((i3 * i4) * 3) / 2) {
            Logging.m1369e(TAG, "Insufficient output buffer size: " + bufferInfo.size);
            return;
        }
    ...
    private void reformat(MediaFormat mediaFormat) {
        ...
        int integer2;
        this.outputThreadChecker.checkIsOnValidThread();
        Logging.m1368d(TAG, "Decoder format changed: " + mediaFormat);
        ...
        synchronized (this.dimensionLock) {
            try {
                if (integer != this.width || integer2 != this.height) {
                    ...
                    Logging.m1372w(TAG, "Unexpected format dimensions. Configured " + this.width + "*" + this.height + ". New " + integer + "*" + integer2 + ". Skip it");
                    return;
                }
                if (this.surfaceTextureHelper == null && mediaFormat.containsKey("color-format")) {
                    this.colorFormat = mediaFormat.getInteger("color-format");
                    Logging.m1368d(TAG, "Color: 0x" + Integer.toHexString(this.colorFormat));
                ...
                synchronized (this.dimensionLock) {
                    try {
                        ...
                        Logging.m1368d(TAG, "Frame stride and slice height: " + this.stride + " x " + this.sliceHeight);
    ...
    public void releaseCodecOnOutputThread() {
        ...
        try {
            ...
            Logging.m1370e(TAG, "Media decoder stop failed", e);
        }
        try {
            this.codec.release();
        } catch (Exception e2) {
            Logging.m1370e(TAG, "Media decoder release failed", e2);
