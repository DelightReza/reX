...
public class Camera1Enumerator implements CameraEnumerator {
    ...
    public String[] getDeviceNames() {
        ...
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
            ...
            if (deviceName != null) {
                ...
                Logging.m1368d(TAG, "Index: " + i + ". " + deviceName);
            } else {
                Logging.m1369e(TAG, "Index: " + i + ". Failed to query camera name.");
            }
        }
    ...
    private static Camera.CameraInfo getCameraInfo(int i) {
        ...
        try {
            ...
            Logging.m1370e(TAG, "getCameraInfo failed on index " + i, e);
            return null;
        }
    }
    ...
    private static List<CameraEnumerationAndroid.CaptureFormat> enumerateFormats(int i) {
        int i2;
        Logging.m1368d(TAG, "Get supported formats for camera index " + i + ".");
        long jElapsedRealtime = SystemClock.elapsedRealtime();
        Camera cameraOpen = null;
        try {
            try {
                Logging.m1368d(TAG, "Opening camera with index " + i);
                ...
                try {
                    ...
                    Logging.m1370e(TAG, "getSupportedFormats() failed on camera index " + i, e);
                }
                Logging.m1368d(TAG, "Get supported formats for camera index " + i + " done. Time spent: " + (SystemClock.elapsedRealtime() - jElapsedRealtime) + " ms.");
                return arrayList;
            } catch (RuntimeException e2) {
                Logging.m1370e(TAG, "Open camera failed on camera index " + i, e2);
    ...
    static int getCameraIndex(String str) {
        Logging.m1368d(TAG, "getCameraIndex: " + str);
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
