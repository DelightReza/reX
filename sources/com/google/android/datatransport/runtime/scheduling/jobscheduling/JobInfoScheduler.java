...
public class JobInfoScheduler implements WorkScheduler {
    ...
    public void schedule(TransportContext transportContext, int i, boolean z) {
        ...
        if (!z && isJobServiceOn(jobScheduler, jobId, i)) {
            Logging.m251d("JobInfoScheduler", "Upload for context %s is already scheduled. Returning...", transportContext);
            return;
        }
        ...
        persistableBundle.putInt(PluginsConstants.MenuItemProperties.PRIORITY, PriorityMapping.toInt(transportContext.getPriority()));
        if (transportContext.getExtras() != null) {
        ...
        Logging.m252d("JobInfoScheduler", "Scheduling upload for context %s with jobId=%d in %dms(Backend next call timestamp %d). Attempt %d", transportContext, Integer.valueOf(jobId), Long.valueOf(this.config.getScheduleDelay(transportContext.getPriority(), nextCallTime, i)), Long.valueOf(nextCallTime), Integer.valueOf(i));
