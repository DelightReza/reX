...
public final class FrameworkMediaDrm implements ExoMediaDrm {
    public static final ExoMediaDrm.Provider DEFAULT_PROVIDER = new ExoMediaDrm.Provider() { // from class: com.google.android.exoplayer2.drm.FrameworkMediaDrm$$ExternalSyntheticLambda0
        ...
        public final ExoMediaDrm acquireExoMediaDrm(UUID uuid) {
            return FrameworkMediaDrm.m2383$r8$lambda$3GNQEqPU_wUEQYVzGhnf5fwpY(uuid);
        }
    };
    ...
    /* renamed from: $r8$lambda$3GNQ--EqPU_wUEQYVzGhnf5fwpY, reason: not valid java name */
    public static /* synthetic */ ExoMediaDrm m2383$r8$lambda$3GNQEqPU_wUEQYVzGhnf5fwpY(UUID uuid) {
        try {
            return newInstance(uuid);
        } catch (UnsupportedDrmException unused) {
            Log.m286e("FrameworkMediaDrm", "Failed to instantiate a FrameworkMediaDrm for uuid: " + uuid + ".");
    ...
    private FrameworkMediaDrm(UUID uuid) {
        ...
        Assertions.checkArgument(!AbstractC0933C.COMMON_PSSH_UUID.equals(uuid), "Use C.CLEARKEY_UUID instead");
        this.uuid = uuid;
        MediaDrm mediaDrm = new MediaDrm(adjustUuid(uuid));
        this.mediaDrm = mediaDrm;
        this.referenceCount = 1;
        if (AbstractC0933C.WIDEVINE_UUID.equals(uuid) && needsForceWidevineL3Workaround()) {
    ...
    public void setPlayerIdForSession(byte[] bArr, PlayerId playerId) {
        if (Util.SDK_INT >= 31) {
            try {
                ...
                Log.m290w("FrameworkMediaDrm", "setLogSessionId failed.");
            }
        }
    }
    ...
    public ExoMediaDrm.KeyRequest getKeyRequest(byte[] bArr, List list, int i, HashMap map) throws NotProvisionedException {
        ...
        return new ExoMediaDrm.KeyRequest(bArrAdjustRequestData, strAdjustLicenseServerUrl, Util.SDK_INT >= 23 ? keyRequest.getRequestType() : TLObject.FLAG_31);
    }
    ...
    public byte[] provideKeyResponse(byte[] bArr, byte[] bArr2) throws JSONException {
        if (AbstractC0933C.CLEARKEY_UUID.equals(this.uuid)) {
    ...
    private static DrmInitData.SchemeData getSchemeData(UUID uuid, List list) {
        if (!AbstractC0933C.WIDEVINE_UUID.equals(uuid)) {
    ...
    private static UUID adjustUuid(UUID uuid) {
        return (Util.SDK_INT >= 27 || !AbstractC0933C.CLEARKEY_UUID.equals(uuid)) ? uuid : AbstractC0933C.COMMON_PSSH_UUID;
    }
    ...
    */
    private static byte[] adjustRequestInitData(java.util.UUID r3, byte[] r4) {
        /*
            java.util.UUID r0 = com.google.android.exoplayer2.AbstractC0933C.PLAYREADY_UUID
        ...
        L18:
            ...
            r2 = 23
            if (r1 >= r2) goto L26
            java.util.UUID r1 = com.google.android.exoplayer2.AbstractC0933C.WIDEVINE_UUID
    ...
    private static String adjustRequestMimeType(UUID uuid, String str) {
        return (Util.SDK_INT < 26 && AbstractC0933C.CLEARKEY_UUID.equals(uuid) && ("video/mp4".equals(str) || "audio/mp4".equals(str))) ? "cenc" : str;
    }

    private static byte[] adjustRequestData(UUID uuid, byte[] bArr) {
        return AbstractC0933C.CLEARKEY_UUID.equals(uuid) ? ClearKeyUtil.adjustRequestData(bArr) : bArr;
    }
    ...
    private static byte[] addLaUrlAttributeIfMissing(byte[] bArr) {
        ...
        if (littleEndianShort != 1 || littleEndianShort2 != 1) {
            Log.m288i("FrameworkMediaDrm", "Unexpected record count or type. Skipping LA_URL workaround.");
            return bArr;
        }
        ...
        if (iIndexOf == -1) {
            Log.m290w("FrameworkMediaDrm", "Could not find the </DATA> tag. Skipping LA_URL workaround.");
        }
    ...
    private static class Api31 {
        ...
        public static void setLogSessionIdOnMediaDrmSession(MediaDrm mediaDrm, byte[] bArr, PlayerId playerId) {
            ...
            FrameworkMediaDrm$Api31$$ExternalSyntheticApiModelOutline0.m271m(Assertions.checkNotNull(mediaDrm.getPlaybackComponent(bArr))).setLogSessionId(logSessionId);
        }
    }
}
