...
public final class HttpMediaDrmCallback implements MediaDrmCallback {
    ...
    public byte[] executeKeyRequest(UUID uuid, ExoMediaDrm.KeyRequest keyRequest) throws MediaDrmCallbackException {
        ...
        if (TextUtils.isEmpty(licenseServerUrl)) {
            ...
            Uri uri = Uri.EMPTY;
            throw new MediaDrmCallbackException(builder.setUri(uri).build(), uri, ImmutableMap.m441of(), 0L, new IllegalStateException("No license URL"));
        }
        HashMap map = new HashMap();
        UUID uuid2 = AbstractC0933C.PLAYREADY_UUID;
        if (uuid2.equals(uuid)) {
            str = "text/xml";
        } else {
            str = AbstractC0933C.CLEARKEY_UUID.equals(uuid) ? "application/json" : "application/octet-stream";
        }
