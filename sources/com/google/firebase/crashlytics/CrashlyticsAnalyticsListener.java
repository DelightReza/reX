...
class CrashlyticsAnalyticsListener implements AnalyticsConnector.AnalyticsConnectorListener {
    ...
    public void onMessageTriggered(int i, Bundle bundle) {
        String string;
        Logger.getLogger().m457v(String.format(Locale.US, "Analytics listener received message. ID: %d, Extras: %s", Integer.valueOf(i), bundle));
        if (bundle == null || (string = bundle.getString("name")) == null) {
            return;
        }
        Bundle bundle2 = bundle.getBundle(PluginsConstants.PARAMS);
        if (bundle2 == null) {
            bundle2 = new Bundle();
        }
