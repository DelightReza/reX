...
public class CrashlyticsReportPersistence {
    ...
    public void persistReport(CrashlyticsReport crashlyticsReport) {
        ...
        if (session == null) {
            Logger.getLogger().m451d("Could not get session for report");
            return;
        }
        ...
        try {
            ...
            Logger.getLogger().m452d("Could not persist report for session " + identifier, e);
        }
    }
    ...
    public void persistEvent(CrashlyticsReport.Session.Event event, String str, boolean z) {
        ...
        try {
            ...
            Logger.getLogger().m460w("Could not persist event for session " + str, e);
        }
        trimEvents(str, i);
    }
    ...
    public void finalizeReports(String str, long j) {
        for (String str2 : capAndGetOpenSessions(str)) {
            Logger.getLogger().m457v("Finalizing report for session " + str2);
    ...
    public void finalizeSessionWithNativeEvent(String str, CrashlyticsReport.FilesPayload filesPayload, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        ...
        Logger.getLogger().m451d("Writing native session report for " + str + " to file: " + sessionFile);
    ...
    public List loadFinalizedReports() {
        ...
        for (File file : allFinalizedReportFiles) {
            try {
                ...
                Logger.getLogger().m460w("Could not load report file " + file + "; deleting", e);
                file.delete();
            }
        }
        return arrayList;
    ...
    private SortedSet capAndGetOpenSessions(String str) {
        ...
        if (openSessionIds.size() > 8) {
            while (openSessionIds.size() > 8) {
                ...
                Logger.getLogger().m451d("Removing session over cap: " + str2);
                this.fileStore.deleteSessionFiles(str2);
    ...
    private void synthesizeReport(String str, long j) {
        ...
        if (sessionFiles.isEmpty()) {
            Logger.getLogger().m457v("Session " + str + " has no events.");
            return;
        }
        ...
        loop0: while (true) {
            ...
            for (File file : sessionFiles) {
                try {
                    ...
                    Logger.getLogger().m460w("Could not add event to report for " + file, e);
                }
        ...
        if (arrayList.isEmpty()) {
            Logger.getLogger().m459w("Could not parse event files for session " + str);
            return;
        }
    ...
    private void synthesizeNativeReportFile(File file, CrashlyticsReport.FilesPayload filesPayload, String str, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        try {
            ...
            Logger.getLogger().m460w("Could not synthesize final native report file for " + file, e);
        }
    }

    private void synthesizeReportFile(File file, List list, long j, boolean z, String str) {
        File report;
        try {
            CrashlyticsReportJsonTransform crashlyticsReportJsonTransform = TRANSFORM;
            ...
            Logger.getLogger().m460w("Could not synthesize final report file for " + file, e);
        }
    }
