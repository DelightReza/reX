...
final class ReportQueue {
        ...
        synchronized (this.queue) {
            try {
                ...
                if (z) {
                    ...
                    if (isQueueAvailable()) {
                        Logger.getLogger().m451d("Enqueueing report: " + crashlyticsReportWithSessionId.getSessionId());
                        Logger.getLogger().m451d("Queue size: " + this.queue.size());
                        this.singleThreadExecutor.execute(new ReportRunnable(crashlyticsReportWithSessionId, taskCompletionSource));
                        Logger.getLogger().m451d("Closing task for report: " + crashlyticsReportWithSessionId.getSessionId());
                        taskCompletionSource.trySetResult(crashlyticsReportWithSessionId);
                        return taskCompletionSource;
                    }
                    calcStep();
                    Logger.getLogger().m451d("Dropping report due to queue being full: " + crashlyticsReportWithSessionId.getSessionId());
    ...
    public void sendReport(final CrashlyticsReportWithSessionId crashlyticsReportWithSessionId, final TaskCompletionSource taskCompletionSource) {
        Logger.getLogger().m451d("Sending report through Google DataTransport: " + crashlyticsReportWithSessionId.getSessionId());
    ...
    private final class ReportRunnable implements Runnable {
        ...
        public void run() throws InterruptedException {
            ...
            Logger.getLogger().m451d("Delay for: " + String.format(Locale.US, "%.2f", Double.valueOf(dCalcDelay / 1000.0d)) + " s for report: " + this.reportWithSessionId.getSessionId());
