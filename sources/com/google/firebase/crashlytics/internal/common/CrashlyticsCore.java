...
public class CrashlyticsCore {
    ...
    public boolean onPreExecute(AppData appData, SettingsProvider settingsProvider) {
        ...
        try {
            ...
            if (zDidPreviousInitializationFail && CommonUtils.canTryConnection(this.context)) {
                Logger.getLogger().m451d("Crashlytics did not finish previous background initialization. Initializing synchronously.");
                finishInitSynchronously(settingsProvider);
                return false;
            }
            Logger.getLogger().m451d("Successfully configured exception handler.");
            return true;
        } catch (Exception e) {
            Logger.getLogger().m454e("Crashlytics was not started due to an exception during initialization", e);
    ...
    public Task doBackgroundInitialization(SettingsProvider settingsProvider) {
        ...
        try {
            ...
            if (!settingsProvider.getSettingsSync().featureFlagData.collectReports) {
                Logger.getLogger().m451d("Collection of crash reports disabled in Crashlytics settings.");
                return Tasks.forException(new RuntimeException("Collection of crash reports disabled in Crashlytics settings."));
            }
            if (!this.controller.finalizeSessions(settingsProvider)) {
                Logger.getLogger().m459w("Previous sessions could not be finalized.");
            }
            return this.controller.submitAllReports(settingsProvider.getSettingsAsync());
        } catch (Exception e) {
            Logger.getLogger().m454e("Crashlytics encountered a problem during asynchronous initialization.", e);
    ...
    private void finishInitSynchronously(final SettingsProvider settingsProvider) throws ExecutionException, InterruptedException, TimeoutException {
        ...
        Logger.getLogger().m451d("Crashlytics detected incomplete initialization on previous app launch. Will initialize synchronously.");
        try {
            futureSubmit.get(3L, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            Logger.getLogger().m454e("Crashlytics was interrupted during initialization.", e);
        } catch (ExecutionException e2) {
            Logger.getLogger().m454e("Crashlytics encountered a problem during initialization.", e2);
        } catch (TimeoutException e3) {
            Logger.getLogger().m454e("Crashlytics timed out during initialization.", e3);
        }
    }
    ...
    void markInitializationStarted() {
        ...
        Logger.getLogger().m457v("Initialization marker file was created.");
    }
        ...
        this.backgroundWorker.submit(new Callable() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsCore.3
            ...
            public Boolean call() {
                try {
                    ...
                    if (!zRemove) {
                        Logger.getLogger().m459w("Initialization marker file was not properly removed.");
                    }
                    return Boolean.valueOf(zRemove);
                } catch (Exception e) {
                    Logger.getLogger().m454e("Problem encountered deleting Crashlytics initialization marker.", e);
    ...
    static boolean isBuildIdValid(String str, boolean z) {
        if (!z) {
            Logger.getLogger().m457v("Configured not to require a build ID.");
            return true;
        }
