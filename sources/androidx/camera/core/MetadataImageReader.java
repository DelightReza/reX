        ...
        synchronized (this.mLock) {
            try {
                if (this.mMatchedImageProxies.size() < getMaxImages()) {
                    ...
                    Logger.m43d("TAG", "Maximum image number reached.");
        ...
        synchronized (this.mLock) {
            try {
                ...
                if (size >= imageReaderProxy.getMaxImages()) {
                    Logger.m43d("MetadataImageReader", "Skip to acquire the next image because the acquired image count has reached the max images count.");
                    return;
                }
                do {
                    try {
                        ...
                        Logger.m44d("MetadataImageReader", "Failed to acquire next image.", e);
