...
public class QuirkSettingsLoader implements Function {
    ...
    public QuirkSettings apply(Context context) {
        try {
            ...
            if (bundle == null) {
                Logger.m48w("QuirkSettingsLoader", "No metadata in MetadataHolderService.");
                return null;
            }
            return buildQuirkSettings(context, bundle);
        } catch (PackageManager.NameNotFoundException unused) {
            Logger.m43d("QuirkSettingsLoader", "QuirkSettings$MetadataHolderService is not found.");
            return null;
        }
    }
    ...
    private static QuirkSettings buildQuirkSettings(Context context, Bundle bundle) {
        ...
        Logger.m43d("QuirkSettingsLoader", "Loaded quirk settings from metadata:");
        Logger.m43d("QuirkSettingsLoader", "  KEY_DEFAULT_QUIRK_ENABLED = " + z);
        Logger.m43d("QuirkSettingsLoader", "  KEY_QUIRK_FORCE_ENABLED = " + Arrays.toString(strArrLoadQuirks));
        Logger.m43d("QuirkSettingsLoader", "  KEY_QUIRK_FORCE_DISABLED = " + Arrays.toString(strArrLoadQuirks2));
    ...
    private static String[] loadQuirks(Context context, Bundle bundle, String str) {
        ...
        if (i == -1) {
            Logger.m48w("QuirkSettingsLoader", "Resource ID not found for key: " + str);
            return new String[0];
        }
        try {
            return context.getResources().getStringArray(i);
        } catch (Resources.NotFoundException e) {
            Logger.m49w("QuirkSettingsLoader", "Quirk class names resource not found: " + i, e);
            return new String[0];
        }
    }
    ...
    private static Class resolveQuirkName(String str) throws ClassNotFoundException {
        try {
            ...
            Logger.m48w("QuirkSettingsLoader", str + " does not implement the Quirk interface.");
            return null;
        } catch (ClassNotFoundException e) {
            Logger.m49w("QuirkSettingsLoader", "Class not found: " + str, e);
            return null;
        }
    }
}
