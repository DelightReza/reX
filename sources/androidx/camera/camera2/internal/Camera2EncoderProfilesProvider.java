...
public class Camera2EncoderProfilesProvider implements EncoderProfilesProvider {
    ...
    public Camera2EncoderProfilesProvider(String str, Quirks quirks) throws NumberFormatException {
        ...
        try {
            ...
            Logger.m48w("Camera2EncoderProfilesProvider", "Camera id is not an integer: " + str + ", unable to create Camera2EncoderProfilesProvider");
            z = false;
            i = -1;
        }
    ...
    private EncoderProfilesProxy getProfilesInternal(int i) {
        if (Build.VERSION.SDK_INT >= 31) {
            ...
            if (DeviceQuirks.get(InvalidVideoProfilesQuirk.class) != null) {
                Logger.m43d("Camera2EncoderProfilesProvider", "EncoderProfiles contains invalid video profiles, use CamcorderProfile to create EncoderProfilesProxy.");
            } else {
                try {
                    return EncoderProfilesProxyCompat.from(all);
                } catch (NullPointerException e) {
                    Logger.m49w("Camera2EncoderProfilesProvider", "Failed to create EncoderProfilesProxy, EncoderProfiles might  contain invalid video profiles. Use CamcorderProfile instead.", e);
                }
            }
        }
    ...
    private EncoderProfilesProxy createProfilesFromCamcorderProfile(int i) {
        ...
        try {
            ...
            Logger.m49w("Camera2EncoderProfilesProvider", "Unable to get CamcorderProfile by quality: " + i, e);
