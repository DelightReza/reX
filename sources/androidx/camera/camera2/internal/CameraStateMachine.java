...
class CameraStateMachine {
    ...
    public void updateState(CameraInternal.State state, CameraState.StateError stateError) {
        ...
        switch (C01271.$SwitchMap$androidx$camera$core$impl$CameraInternal$State[state.ordinal()]) {
            case 1:
        ...
        Logger.m43d("CameraStateMachine", "New public camera state " + cameraStateOnCameraPendingOpen + " from " + state + " and " + stateError);
        if (Objects.equals((CameraState) this.mCameraStates.getValue(), cameraStateOnCameraPendingOpen)) {
            return;
        }
        Logger.m43d("CameraStateMachine", "Publishing new public camera state " + cameraStateOnCameraPendingOpen);
