...
public class VideoTimebaseConverter {
    ...
    public long convertToUptimeUs(long j) {
        ...
        int i = C02571.$SwitchMap$androidx$camera$core$impl$Timebase[this.mResolvedInputTimebase.ordinal()];
        if (i != 1) {
            if (i == 2) {
                return j;
            }
        ...
        if (this.mUptimeToRealtimeOffsetUs == -1) {
            ...
            Logger.m43d("VideoTimebaseConverter", "mUptimeToRealtimeOffsetUs = " + this.mUptimeToRealtimeOffsetUs);
        }
    ...
    private Timebase resolveInputTimebase(long j) {
        ...
        if (this.mCameraUseInconsistentTimebaseQuirk != null) {
            Logger.m48w("VideoTimebaseConverter", "CameraUseInconsistentTimebaseQuirk is enabled");
            z = false;
        } else {
        ...
        if (z && timebase != this.mInputTimebase) {
            ...
            Logger.m45e("VideoTimebaseConverter", String.format("Detected camera timebase inconsistent. Please file an issue at https://issuetracker.google.com/issues/new?component=618491&template=1257717 with this error message [Manufacturer: %s, Model: %s, Hardware: %s, API Level: %d%s].\nCamera timebase is inconsistent. The timebase reported by the camera is %s, but the actual timebase contained in the frame is detected as %s.", Build.MANUFACTURER, Build.MODEL, Build.HARDWARE, Integer.valueOf(i), str, this.mInputTimebase, timebase));
            return timebase;
        }
        Logger.m43d("VideoTimebaseConverter", "Detect input timebase = " + timebase);
        return timebase;
    }
